...
int receiveProductList_C6() {
    ...
    int pid = getpid();
    int rc = msgReceive(msgId, &msg, pid);
    if (rc < 0) {
        so_error("C6", "");
        return result;
    }
    ...
    while (msg.msgData.infoProduto.idProduto != FIM_LISTA_PRODUTOS) {
        //6.3 Produtos
        int idProduto = msg.msgData.infoProduto.idProduto;
        char *nomeProduto = msg.msgData.infoProduto.nomeProduto;
        char *categoria = msg.msgData.infoProduto.categoria;
        int preco = msg.msgData.infoProduto.preco;
        

        //6.4 stdout info
        printf("%d %s %s %d\n", idProduto, nomeProduto, categoria, preco);
        so_success("C6.4", "%d %s %s %d", idProduto, nomeProduto, categoria, preco);

        //C6.5 Recebe a msg
        rc = msgReceive(msgId, &msg, pid);
        if (rc < 0) {
            so_error("C6.5", "Failed to receive message from message queue");
            return result;
        }
    }
...
int getIdProdutoUtilizador_C7() {
    ...
    if(result < 0){
        ...
        msg.msgData.infoProduto.idProduto = result;
