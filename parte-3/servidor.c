...
int initShm_S1() {
    ...
    //S1.3
    shmId = shmget(IPC_KEY, sizeof(*db), IPC_CREAT | 0666);
    if (shmId == -1) {
    ...
    if(db != NULL){
        ...
        FILE* f = fopen(FILE_DATABASE_USERS, "r");
        if(!f){
            so_error("S1.5", "");
            return RET_ERROR;
        }
        fread(db->listUsers, sizeof(Login), MAX_USERS, f);
        if(ferror(f)){
            so_error("S1.5", "");
            return RET_ERROR;
        }
...
int initSem_S3() {
    ...
    if (semNrSetValue(semId, SEM_PRODUCTS, 1) == -1 
        || semNrSetValue(semId, SEM_USERS, 1) == -1 
        || semNrSetValue(semId, SEM_NR_SRV_DEDICADOS, 0) == -1) {
        so_error("S3.3", "");
        return result;
    } else {
...
int triggerSignals_S4() {
    ...
    signal(SIGINT, handleSignalSIGINT_S8);
...
void shutdownAndExit_S7() {
    ...
    // S7.2: Envia o sinal SIGUSR1 para cada Servidor Dedicado associado a um utilizador
    for (int i = 0; i < MAX_USERS; i++) {
    ...
    // S7.4: Reescreve o ficheiro bd_produtos.dat apenas com produtos que tenham stock > 0
    FILE* file = fopen(FILE_DATABASE_PRODUCTS, "wb");
    if (file != NULL) {
        for (int i = 0; i < MAX_PRODUCTS; i++) {
    ...
    // S7.5: Apaga todos os elementos IPC (SHM, SEM, MSG) com a KEY IPC_KEY
...
void handleSignalSIGCHLD_S9(int sinalRecebido) {
    ...
    int pidt = wait(NULL);  
    so_success("S9", "Terminou Servidor Dedicado %d", pidt);
...
int triggerSignals_SD10() {
    ...
    signal(SIGUSR1, handleSignalSIGUSR1_SD18);
    result = RET_SUCCESS;
    so_success("SD10", "");
...
int searchUserDB_SD12(Login request) {
    ...
    if (msgSend(msgId, msg, request.pidCliente) == -1) {
        so_error("SD12.2", "");
        return RET_ERROR;
    } else {
        so_success("SD12.2", "");
...
int sendProductList_SD14(int indexClient, int pidCliente) {
 ...
 // SD14.1
    ...
    so_success("SD14.1", "(SD14.1) %s %d %d", msg.msgData.infoLogin.nome, msg.msgData.infoLogin.saldo, msg.msgData.infoLogin.pidServidorDedicado);
...
MsgContent receiveClientOrder_SD15() {
    ...
    if (msgrcv(msgId, &msg, sizeof(msg.msgData), getpid(), 0) == RET_ERROR) {
        so_error("SD15", "");
        return msg;
    }
    so_success("SD15", "");
...
void handleSignalSIGUSR1_SD18(int sinalRecebido) {
    ...
    if (db != NULL) {
        ...
        if (kill(db->listUsers[indexClient].pidCliente, SIGUSR2) != 0) {
            so_error("SD18", "");
        } else {
            so_success("SD18", "");
            shutdownAndExit_SD17();
