...
Login getDadosPedidoUtilizador_C3() {
    ...
    so_debug("<");
    int tempnif = scanf("%d", &request.nif);
    char tempsenha[50];
    int temp2 = scanf("%s", tempsenha);

    if (temp2 != 1 || tempnif != 1){
        request.nif = -1;
        so_error("C3", "");
    } else {
        request.pid_cliente = getpid();
        //request.nif = tempnif;
        strcpy(request.senha, tempsenha);
 ...
 *              erro na bytesEscritos, dá so_error, e vai para o passo C11, caso contrário, dá
 *              so_success;
 * @param nameFifo O nome do FIFO do servidor (i.e., FILE_REQUESTS)
 * @param Login Elemento com os dados a enviar
 * @return int Sucesso (0: success, -1: error)
 */
...
Login readAckLogin_C7(char *nameFifo) {
    ...
    if(!fifo){
        ...
        if(fread(&ackLogin, sizeof(ackLogin), 1, fifo) == 1 ){
...
int sleepRandomTime_C9() {
    ...
    int intervalo = MAX_PROCESSAMENTO -  MIN_PROCESSAMENTO + 1;
    tempoDeProcessamento = MIN_PROCESSAMENTO + (so_rand() % intervalo);
 ...
 *              bytesEscritos na diretoria local, escreve nesse FIFO (acesso sequencial) a string
...
int writeFimSessao_C10(char *nameFifo, int tempoDeProcessamento, Login ackLogin) {
    ...
    char texto[100];
    sprintf(texto, "Sessão Login ativa durante %d segundos", tempoDeProcessamento);
    if(fwrite(texto, sizeof(char), strlen(texto), fifo) < strlen(texto)){
        so_error("C10", "");
        deleteFifoAndExit_C11();
        fclose(fifo);
    }
...
void trataSinalSIGINT_C13(int sinalRecebido) {
    ...
    if(buildNomeFifo(nameFifoServidorDedicado, 20, FILE_PREFIX_SRVDED, clientRequest.pid_servidor_dedicado, FILE_SUFFIX_FIFO) == -1){
        so_error("C13", "");
    } else {
        FILE *fifo = fopen(nameFifoServidorDedicado, "w");
        if(fifo){
            if(fwrite("Sessão cancelada pelo utilizador", sizeof(char), strlen("Sessão cancelada pelo utilizador"), fifo) < 0){
                so_error("C13", "");
            } else {
                so_success("C13", "");
                fclose(fifo);
