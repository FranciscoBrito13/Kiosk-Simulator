...
Login readRequest_S4(char *nameFifo) {
    ...
    FILE *f = fopen(nameFifo, "rb");
    if(!f){
        so_error("S4", "");
    }else {

        int a = fread(&itemDB, sizeof(Login), 1, f);
        if(itemDB.nif == -1 || a == -1){
            so_error("S4", "");
            itemDB.nif = -1;
        }else{
...
int createServidorDedicado_S5() {
    ...
    if (result > 0){
        //É pai 
        so_success("S5", "Servidor Dedicado: PID %d", result);
...
void trataSinalSIGINT_S7(int sinalRecebido) {
    ...
    if( f == NULL ){
        ...
        while(fread(&itemDB, sizeof(Login), 1, f) == 1){
            if(itemDB.nif == 0){
                so_error("S7.2", "");
                deleteFifoAndExit_S6();
            }
            ...
            if(itemDB.pid_servidor_dedicado > 0){
                if(kill(itemDB.pid_servidor_dedicado, SIGUSR1) == -1)
                    so_error("S7.3", "");
                    deleteFifoAndExit_S6();
            }
            itemDB.nif = 0;
...
void trataSinalSIGCHLD_S8(int sinalRecebido) {
    ...
    int pidt = wait(NULL);  //identifica o pid do servidor dedicado que foi terminado
    so_success("S8", "Terminou Servidor Dedicado %d", pidt);
...
int triggerSignals_SD9() {
    ...
    signal(SIGUSR1, trataSinalSIGUSR1_SD18);
    result = 0;
    so_success("SD9", "");

    // if(signal(SIGINT, SIG_IGN) && signal(SIGUSR1, trataSinalSIGUSR1_SD18)){
    //     so_success("SD9", "");
    //     result = 0; 
    // }else{
    //     so_error("SD9", "");
...
int procuraUtilizadorBD_SD11(Login clientRequest, char *nameDB, Login *itemDB) {
    ...
    // SD11: Abre o ficheiro nameDB para leitura
    ...
    // SD11.2: Lê o arquivo até o final ou até encontrar o itemDB
    while(fread(itemDB, sizeof(Login), 1, f)){
        // SD11.4: Verifica se o NIF corresponde ao NIF do elemento Login do ficheiro
        if (itemDB->nif == clientRequest.nif){
            so_success("SD11.5", "%d", index_client+1);
            // SD11.5: Verifica se a senha corresponde à senha do elemento Login do ficheiro
            if (strcmp(itemDB->senha,clientRequest.senha) == 0){    
                fclose(f);
                so_success("SD11", "");
                index_client++;
                return index_client;
...
int reservaUtilizadorBD_SD12(Login *clientRequest, char *nameDB, int index_client, Login itemDB) {
    ...
    if (index_client < 0){
        ...
        //Copia o nome e o saldo para a base de dados
        strcpy(clientRequest->nome, itemDB.nome);
    ...
    if (result == 0) {
        ...
        int a = index_client * sizeof(Login); 
        ...
        if(fwrite(clientRequest, sizeof(Login), 1, fp) != 1){
            so_error("SD12.4", "");
            fclose(fp);
            return -1;
        }
...
int buildNomeFifo(char *buffer, int buffer_size, char *prefix, int pid, char *suffix) {
    ...
    int result = snprintf(buffer, buffer_size, "%s%d%s", prefix, pid, suffix);
...
int createFifo_SD13(char *nameFifo) {
    ...
    if(mkfifo(nameFifo, S_IRUSR | S_IWUSR) == -1){
        so_error("SD13", "");
        return result;
    }
    result = 0;
...
int sendAckLogin_SD14(Login ackLogin, char *nameFifo) {
    ...
    FILE *fifo = fopen(nameFifo, "wb");

    if (!fifo){
        so_error("SD14", "A abrir fifo");
        deleteFifoAndExit_SD17();
        return result;
    }

    if(fwrite(&ackLogin, sizeof(Login), 1, fifo) != 1){
        so_error("SD14", "A escrever no fifo");
        deleteFifoAndExit_SD17();
        fclose(fifo);
        return result;
    }
...
int readFimSessao_SD15(char *nameFifo) {
    ...
    FILE *fifo = fopen(nameFifo, "rb");
    if(!fifo){
        so_error("SD15", "Erro a abrir o fifo");
        deleteFifoAndExit_SD17();
        return result;
    }
    char str[512];
    ...
    while (fread(&l, sizeof(char), 1, fifo) > 0 && l != '\0'){
        str[charLidos] = l;
        charLidos++;
    }
...
int terminaSrvDedicado_SD16(Login *clientRequest, char *nameDB, int index_client) {
    ...
    // SD16.3
    if (fseek(bd, index_client * sizeof(Login), SEEK_SET) != 0) {
        so_error("SD16", "");
        fclose(bd);
        return result;
    }
    ...
    // SD16.4
    if (fwrite(clientRequest, sizeof(Login), 1, bd) != 1) {
        so_error("SD16", "");
        fclose(bd);
        return result;
    }
...
void deleteFifoAndExit_SD17() {
    ...
    int rem = unlink(nameFifoServidorDedicado); 
    if (rem == -1) {
        so_error("SD17", "");
    } else {
        so_success("SD17", "");
...
void trataSinalSIGUSR1_SD18(int sinalRecebido) {
    ...
    if(kill(clientRequest.pid_cliente, SIGUSR2) != 0){
        so_error("SD18", "");
    }
